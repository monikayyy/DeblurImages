"""
Local MAE perceptual loss using ViT-B/16 features.
...
from losses import contrast_loss
from torchvision.transforms.functional import normalize
#from loss.pdl import projectedDistributionLoss
from losses import pos_embed, contrast_loss 
...
class ReconstructPerceptualLoss(nn.Module):
    def __init__(self, opt):
        ...
        pos_embed.interpolate_pos_embed(self.pretrain_mae, checkpoint_model)
        self.pretrain_mae.load_state_dict(checkpoint_model, strict=False)
        for _, p in self.pretrain_mae.named_parameters():
            p.requires_grad = False
        self.constrastive_loss = contrast_loss.selfPerceptualLoss(img_size)
